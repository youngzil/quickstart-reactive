 ...
 * implied. See the License for the specific language governing permissions and limitations under the License.
 */
package org.quickstart.reactivex.rxjava.async.util.operators;
...
public class OperatorForEachFutureTest {
  ...
  public void testSimple() {
    ...
    try {
      Observable<Integer> source = Observable.just(1, 2, 3).subscribeOn(Schedulers.computation());
  ...
  public void testSimpleThrowing() {
    ...
    try {
      Observable<Integer> source = Observable.<Integer>error(new TestException()).subscribeOn(Schedulers.computation());
