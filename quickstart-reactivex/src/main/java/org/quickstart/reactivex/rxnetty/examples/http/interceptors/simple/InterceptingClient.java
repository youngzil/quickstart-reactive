...
import static org.quickstart.reactivex.rxnetty.examples.http.interceptors.simple.InterceptingServer.INTERCEPTOR_HEADER_NAME;
...
/**
 * A client for {@link InterceptingServer}, which follows a simple text based, new line delimited message protocol. It sends an integer as the request payload and expects two integers separated by new
 * lines, as the response payload.
 *
 * There are three ways of running this example:
 *
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link InterceptingServer}) on an ephemeral port, send a "Hello World!" message to the server and print the
 * response.
 *
...
public final class InterceptingClient {
  ...
  public static void main(String[] args) {
    ...
    HttpClient.newClient(serverAddress).enableWireLogging("inter-client", LogLevel.DEBUG).intercept()
        .next(provider -> (version, method, uri) -> provider.createRequest(version, method, uri).addHeader(INTERCEPTOR_HEADER_NAME, "client"))
        .finish().createGet("/hello").doOnNext(resp -> logger.info(resp.toString()))
        .flatMap((HttpClientResponse<ByteBuf> resp) -> resp.getContent().map(bb -> bb.toString(Charset.defaultCharset()))).toBlocking()
        .forEach(logger::info);
  }
}
