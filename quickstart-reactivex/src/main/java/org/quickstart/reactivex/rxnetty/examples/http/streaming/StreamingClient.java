...
package org.quickstart.reactivex.rxnetty.examples.http.streaming;
 ...
 *
 * This example assumes that the server to which this client connects (using the below mentioned option), sends an infinite stream of HTTP chunks. This client only takes 10 items from that infinite
 ...
 *
 * You can also use this client to send a GET request "/stream" to an existing HTTP server (different than {@link StreamingServer}) by passing the port and host of the existing server similar to the
 * case above:
 *
 * <PRE>
...
public final class StreamingClient {
  ...
  public static void main(String[] args) {
    ...
    HttpClient.newClient(serverAddress).enableWireLogging("streaming-client", LogLevel.DEBUG)
