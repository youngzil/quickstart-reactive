...
package org.quickstart.reactivex.rxnetty.http.hello;

import java.net.SocketAddress;
import java.nio.charset.Charset;

import org.quickstart.reactivex.rxnetty.ExamplesEnvironment;
 ...
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link HelloWorldServer}) on an ephemeral port and then send an HTTP request to that server and print the
 * response.
 *
 * <h2>After starting {@link HelloWorldServer}</h2>
 *
 * If you want to see how {@link HelloWorldServer} work, you can run {@link HelloWorldServer} by yourself and then pass the port on which the server started to this class as a program argument:
 *
 * <PRE>
 ...
 *
 * You can also use this client to send a GET request "/hello" to an existing HTTP server (different than {@link HelloWorldServer}) by passing the port and host of the existing server similar to the
 * case above:
 *
 * <PRE>
...
public final class HelloWorldClient {
  ...
  public static void main(String[] args) {
     ...
     */
    String[] argsargs = {"8080"};
    String[] args2 = {"1", "2", "3"};
    SocketAddress serverAddress = env.getServerAddress(HelloWorldServer.class, argsargs);

    HttpClient.newClient(serverAddress).enableWireLogging("hello-client", LogLevel.ERROR).createGet("/hello")
