 ...
 *
 * The intent here is <em>NOT</em> to prescribe how to write a fully functional proxy, which would otherwise require setting appropriate request headers, do appropriate routing on the origin
 * endpoints, etc. Instead, it is to demonstrate how to write a server that forwards the received request, as is to another server using an RxNetty client.
 *
 * This example starts an embedded target server, which is a simple HTTP server that returns a response with HTTP status of 200 and a content of "Hello World!" for every request it recieves. The proxy
 * server then forwards all received requests to this target server preserving all the request and response headers as well as content. It adds an additional response header "X-Proxied-By" with a
 * value "RxNetty" to demonstrate that the response is a proxied response.
 */
public final class ProxyServer {
  ...
  public static void main(final String[] args) {
    ...
    server = HttpServer.newServer().enableWireLogging("proxy-server", LogLevel.DEBUG).start((serverReq, serverResp) -> {
      /*
       * Create a new HTTP request for the target server, using the method and URI from
       * the server request.
       */
      HttpClientRequest<ByteBuf, ByteBuf> clientReq = targetClient.createRequest(serverReq.getHttpMethod(), serverReq.getUri());
      ...
      return clientReq.writeContent(serverReq.getContent()).flatMap(clientResp -> {
  ...
  private static int startTargetServer() {
    ...
    return HttpServer.newServer().start((req, resp) -> resp.writeString(just("HelloWorld!"))).getServerPort();
  }
}
