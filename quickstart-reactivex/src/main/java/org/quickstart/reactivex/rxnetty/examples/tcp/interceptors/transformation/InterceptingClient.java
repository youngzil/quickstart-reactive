...
/**
 * A client for {@link TransformingInterceptorsServer}, which follows a simple text based, new line delimited message protocol. The client expects a "Hello" as the first message and then next two
 * integers for every integer sent by the client.
 *
 * There are three ways of running this example:
 *
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link TransformingInterceptorsServer}) on an ephemeral port, send a "Hello World!" message to the server and
 * print the response.
 *
 ...
 *
 * You can also use this client to connect to an already running TCP server (different than {@link TransformingInterceptorsServer}) by passing the port and host of the existing server similar to the
 * case above:
 *
 * <PRE>
...
public final class InterceptingClient {
  ...
  public static void main(String[] args) {
    ...
    TcpClient.newClient(serverAddress).<ByteBuf, String>addChannelHandlerLast("string-line-decoder", StringLineDecoder::new).intercept()
        ...
        .nextWithWriteTransform(provider -> () -> provider.newConnectionRequest().map(writeIntegers())).finish().createConnectionRequest()
        .flatMap(connection -> connection.write(Observable.just(1)).cast(Integer.class).mergeWith(connection.getInput())).take(2)
  ...
  private static Func1<Connection<String, ByteBuf>, Connection<Integer, ByteBuf>> readIntegers() {
    return c -> c.transformRead(o -> o.filter(s1 -> !s1.isEmpty()).flatMap(s -> Observable.from(s.split(" ")).map(Integer::parseInt)));
  }
