...
/**
 * A client for {@link TransformingInterceptorsServer}, which follows a simple text based, new line delimited message protocol. It sends an integer as the request payload and expects two integers
 * separated by new lines, as the response payload.
 *
 * There are three ways of running this example:
 *
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link TransformingInterceptorsServer}) on an ephemeral port, send an integer to the server and print the
 * response.
 *
 * <h2>After starting {@link TransformingInterceptorsServer}</h2>
 *
 * If you want to see how {@link TransformingInterceptorsServer} work, you can run {@link TransformingInterceptorsServer} by yourself and then pass the port on which the server started to this class
 ...
 *
 * You can also use this client to send a POST request "/ints" to an existing HTTP server (different than {@link TransformingInterceptorsServer}) by passing the port and host of the existing server
...
public final class InterceptingClient {
  ...
  public static void main(String[] args) {
    ...
    HttpClient.newClient(serverAddress).<ByteBuf, String>addChannelHandlerLast("line-decoder", HttpContentStringLineDecoder::new)
        .enableWireLogging("inter-client", LogLevel.DEBUG).intercept().<Integer, Integer>nextWithTransform(readWriteInts()).finish()
        .createPost("/ints").writeContent(Observable.just(1)).doOnNext(resp -> logger.info(resp.toString())).flatMap(HttpClientResponse::getContent)
        .map(Object::toString).toBlocking().forEach(logger::info);
  }

  private static TransformingInterceptor<ByteBuf, String, Integer, Integer> readWriteInts() {
    return provider -> (version, method, uri) -> provider.createRequest(version, method, uri).transformContent(intToByteBuffer())
        .transformResponseContent(o -> o.filter(s -> !s.isEmpty()).map(Integer::parseInt));
  }
