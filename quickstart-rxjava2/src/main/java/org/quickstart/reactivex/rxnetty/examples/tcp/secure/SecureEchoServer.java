...
package org.quickstart.reactivex.rxnetty.examples.tcp.secure;

import java.nio.charset.Charset;

import org.quickstart.reactivex.rxnetty.examples.ExamplesEnvironment;
import org.quickstart.reactivex.rxnetty.examples.tcp.echo.EchoServer;
...
/**
 * A secure TCP echo server that echoes all input it receives on any connection, after prepending the input with a fixed string. The only difference between this example and {@link EchoServer} is that
 * this adds TLS.
 *
...
public final class SecureEchoServer {
  ...
  public static void main(final String[] args) {
    ...
    TcpServer<ByteBuf, ByteBuf> server =
        TcpServer.newServer().enableWireLogging("echo-server", LogLevel.DEBUG).unsafeSecure().start(connection -> connection
