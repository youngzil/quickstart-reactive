 ...
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link SecureHelloWorldServer}) on an ephemeral port and then send an HTTP request to that server and print
 * the response.
 *
 * <h2>After starting {@link SecureHelloWorldServer}</h2>
 *
 * If you want to see how {@link SecureHelloWorldServer} work, you can run {@link SecureHelloWorldServer} by yourself and then pass the port on which the server started to this class as a program
 * argument:
 *
 * <PRE>
 ...
 *
 * You can also use this client to send a GET request "/hello" to an existing HTTP server (different than {@link SecureHelloWorldServer}) by passing the port and host of the existing server similar to
 * the case above:
 *
 * <PRE>
...
public final class SecureHelloWorldClient {
  ...
  public static void main(String[] args) {
    ...
    HttpClient.newClient(serverAddress).enableWireLogging("inter-client", LogLevel.DEBUG).unsafeSecure().createGet("/hello")
