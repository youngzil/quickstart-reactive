...
import org.quickstart.reactivex.rxnetty.examples.http.interceptors.simple.InterceptingServer;
...
public final class TransformingInterceptorsServer {
  ...
  public static void main(final String[] args) {
    ...
    server = HttpServer.newServer().<String, ByteBuf>addChannelHandlerLast("line-decoder", HttpContentStringLineDecoder::new)
        .enableWireLogging("inter-server", LogLevel.DEBUG).start(HttpServerInterceptorChain.<String, ByteBuf>start()
            .<Integer, Integer>nextWithTransform(readWriteInts()).end(numberIncrementingHandler()));
  ...
  private static RequestHandler<Integer, Integer> numberIncrementingHandler() {
    return (req, resp) -> resp.write(req.getContent().flatMap(integer -> just(integer, ++integer)));
  }

  private static TransformingInterceptor<String, ByteBuf, Integer, Integer> readWriteInts() {
    return handler -> (request, response) -> handler.handle(request.transformContent(stringToInt()),
        response.transformContent(new AllocatingTransformer<Integer, ByteBuf>() {
          ...
          public List<ByteBuf> transform(Integer toTransform, ByteBufAllocator allocator) {
            ...
            return singletonList(allocator.buffer().writeBytes(msg.getBytes()));
          }
        }));
  }
