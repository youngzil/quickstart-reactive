...
import org.quickstart.reactivex.rxnetty.examples.http.ws.messaging.MessageFrame.MessageType;
...
/**
 * An abstraction to demonstrate asynchronous message creation. This class is configured to send a finite number of messages to a {@link UnicastBufferingSubject}
 */
public class MessageProducer {
  ...
  public MessageProducer(int messagesToSend, long interval, TimeUnit intervalDuration) {
    ...
    messageStream = sender.filter(f -> f instanceof MessageFrame).cast(MessageFrame.class).map(messageTracker::addPendingMessage)
        .cast(WebSocketFrame.class).concatWith(Observable.just(new CloseWebSocketFrame()));

    Observable.interval(interval, intervalDuration).take(messagesToSend)
