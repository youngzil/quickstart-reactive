...
package org.quickstart.reactivex.rxnetty.local;
...
public final class LocalEcho {
  ...
  public static void main(String[] args) {
    ...
    TcpServer.newServer(serverAddress, eventLoopGroup, LocalServerChannel.class).enableWireLogging("echo-server", LogLevel.DEBUG)
        .start(connection -> connection
            .writeStringAndFlushOnEach(connection.getInput().map(bb -> bb.toString(Charset.defaultCharset())).map(msg -> "echo => " + msg)));

    /*Create a new client for the server address*/
    TcpClient.newClient(serverAddress, eventLoopGroup, LocalChannel.class).enableWireLogging("echo-client", LogLevel.DEBUG).createConnectionRequest()
