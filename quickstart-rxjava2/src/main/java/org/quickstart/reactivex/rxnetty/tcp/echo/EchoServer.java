...
public final class EchoServer {
  ...
  public static void main(final String[] args) {
    ...
    server = TcpServer.newServer(8080).enableWireLogging("echo-server", LogLevel.DEBUG).start(connection -> connection.writeStringAndFlushOnEach(
        connection.getInput().map(bb -> bb.toString(Charset.defaultCharset())).doOnNext(logger::info).map(msg -> "echo => " + msg)));
