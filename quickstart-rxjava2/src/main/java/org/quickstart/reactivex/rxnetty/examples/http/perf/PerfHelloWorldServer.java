...
import org.quickstart.reactivex.rxnetty.examples.http.helloworld.HelloWorldServer;
...
/**
 * This is an HTTP server example used to do "Hello World" benchmarks. A "Hello World" benchmark is a good benchmark to analyze library overheads as the application code does not do much.
 *
 * This server is not representative of the otherwise normal {@link HelloWorldServer} which is much simpler. This example instead does micro-optimizations like using
 * {@link HttpServerResponse#flushOnlyOnReadComplete()}, setting the content-length header and storing the response content stream, etc. These optimizations reduce the overheads that are usually not
 * significant for applications that do any "real work".
 */
public final class PerfHelloWorldServer {
  ...
  public static void main(final String[] args) {
    ...
    server = HttpServer.newServer()
        .start((req, resp) -> resp.setHeader(CONTENT_LENGTH, CONTENT_LENGTH_HEADER_VAL).flushOnlyOnReadComplete().write(RESPONSE_CONTENT));
