 ...
 *
 * This example assumes that the server to which this client connects (using the below mentioned option), sends an infinite Server Sent Event stream adhering to the specifications. This client only
 * takes 10 items from that infinite stream and then terminates the connection.
 *
 * There are three ways of running this example:
 *
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link HelloSseServer}) on an ephemeral port and then send an HTTP request to that server and print the
 * response.
 *
 * <h2>After starting {@link HelloSseServer}</h2>
 *
 * If you want to see how {@link HelloSseServer} work, you can run {@link HelloSseServer} by yourself and then pass the port on which the server started to this class as a program argument:
 *
 * <PRE>
 ...
 *
 * You can also use this client to send a GET request "/sse" to an existing HTTP server (different than {@link HelloSseServer}) by passing the port and host of the existing server similar to the case
 * above:
 *
 * <PRE>
...
public final class HelloSseClient {
  ...
  public static void main(String[] args) {
    ...
    HttpClient.newClient(serverAddress).enableWireLogging("inter-client", LogLevel.DEBUG).createGet("/sse")
        .doOnNext(resp -> logger.info(resp.toString())).flatMap(HttpClientResponse::getContentAsServerSentEvents).take(10).toBlocking()
