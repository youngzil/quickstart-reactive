...
/**
 * A client for {@link InterceptingServer}, which follows a simple text based, new line delimited message protocol. The client expects a "Hello" as the first message and then the echo of what it sends
 * to the server. This client, sends a "Hello World" to the server and expects an echo.
 *
...
public final class InterceptingClient {
  ...
  public static void main(String[] args) {
    ...
    TcpClient.newClient(serverAddress).<ByteBuf, String>addChannelHandlerLast("string-line-decoder", StringLineDecoder::new).intercept()
        .next(provider -> () -> provider.newConnectionRequest().map(c -> c.transformRead(o -> o.skip(1)))).finish().createConnectionRequest()
        .flatMap(connection -> connection.writeString(Observable.just("Hello World!")).cast(String.class).mergeWith(connection.getInput())).take(1)
